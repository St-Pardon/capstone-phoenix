...
root module (`infra/terraform/`) uses for remote state. It has its **own local state**, which
...
Finally, wire the outputs into the root `backend.tf` (next: the `#2` commit) and run
`terraform init` in `infra/terraform/`.
